...
async def create_book(book: BookCreate, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
    return BookService(session).create(book)
                                 
@inv_router.put("/book/{isbn}", response_model=Book)
async def update_book(isbn: str, new_book :Book, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
...
async def delete_author_by_id(id: int, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)): 
    return AuthorService(session).delete_author_by_id(id) 

@inv_router.post("/author", response_model=AuthorResponse)
async def create_author(author: AuthorCreate, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
    return AuthorService(session).create(author)

@inv_router.put("/author/{author_id}", response_model=Author)
async def update_author(id: int, new_author: Author, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
...
async def delete_genre_by_id(id: int, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)): 
    return GenreService(session).delete_genre_by_id(id) 

@inv_router.post("/genre", response_model=GenreResponse)
async def create_genre(genre: GenreCreate, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
    return GenreService(session).create(genre)

@inv_router.put("/genre/{genre_id}", response_model=Genre)
async def update_genre(id: int, new_genre: Genre, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
...
async def delete_publisher_by_id(id: int, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)): 
    return PublisherService(session).delete_publisher_by_id(id) 

@inv_router.post("/publisher", response_model=PublisherResponse)
async def create_publisher(publisher: PublisherCreate, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
    return PublisherService(session).create(publisher)

@inv_router.put("/publisher/{publisher_id}", response_model=Publisher)
async def update_publisher(id: int, new_publisher: Publisher, current_user: Annotated[User, Depends(UserService.get_current_active_user)], session: Session = Depends(get_session)):
